...
fn core_ch_to_digits(chinese_chars_to_trans:String, dot_position:bool) -> String{
		...
		for i in (0..chinese_chars.chars().count()).rev() {
			...
			match val_from_hash_map{
				Some(val_from_hash_map) => {
					...
					if temp_total == 0  && counting_unit > 10{
...
fn main() {
    println!("{}",core_ch_to_digits("三百四十二".to_string(),false));
}